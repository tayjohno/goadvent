...
// 4. The list is circular; if the current position and the length try to reverse elements beyond
// the end of the list, the operation reverses using as many extra elements as it needs from the
...
func parseLengths(s string) lengths {
	intStrings := strings.Split(s, ",")
	ints := make(lengths, len(intStrings))
	for i := range intStrings {
		ints[i], _ = strconv.Atoi(intStrings[i])
	}
	return ints
}

func parseASCIILengths(s string) lengths {
	bytes := []byte(s)
	ints := make(lengths, len(bytes)+5)
	for i := range bytes {
		ints[i] = int(bytes[i])
	}
	copy(ints[len(bytes):], []int{17, 31, 73, 47, 23})
	return ints
}

const raw string = "129,154,49,198,200,133,97,254,41,6,2,1,255,0,191,108"
